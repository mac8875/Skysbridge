...
(() => {
  ...
  function showMemorial() {
    openModal(`
      ...
      <p>Eingereichte Gedenksterne bleiben privat, bis du die Veröffentlichung beantragst und eine Moderation sie freigibt.</p>
      <form class="form-grid" id="memorialForm">
        <label>Name des Kindes
          <input name="child_name" required maxlength="80">
        </label>
        <label>Deine Erinnerung
        ...
        <label>
          ...
          Einen Platz auf der öffentlichen Sternenhimmel beantragen
        </label>
        <button class="button button-gold">Privat zur Prüfung einreichen</button>
        <div class="notice" id="memorialStatus">Du behältst die Kontrolle über die Veröffentlichung.</div>
      </form>
    `);
    ...
    form.onsubmit = async event => {
      ...
      form.reset();
      setStatus(status, "Der Gedenkstern wurde privat zur Prüfung eingereicht.", "success");
    };
  }
  ...
  async function showStar(slug) {
    ...
    openModal(`
      <article class="star-remembrance">
        ...
        <section class="memory-section" aria-labelledby="memoryHeading">
          <p class="eyebrow">Worte der Erinnerung</p>
          <h3 id="memoryHeading">Eine Erinnerung für ${starName} hinterlassen</h3>
          <p class="memory-intro">
            Deine Worte bleiben privat, bis sie sorgfältig geprüft wurden.
          </p>
          ...
          <form class="form-grid memory-form" id="memoryForm">
            ...
            <label>Eine Erinnerung hinterlassen
              <textarea name="message" maxlength="800" required></textarea>
            </label>
            <button class="button button-gold" type="submit">Erinnerung privat einreichen</button>
            <div class="notice memory-notice" id="memoryStatus">
              Erinnerungen werden geprüft, bevor sie sichtbar werden.
            </div>
          </form>
        </section>
      </article>
    ...
    form.onsubmit = async event => {
      ...
      const { data: { user } } = await db.auth.getUser();
      if (!user) {
        setStatus(status, "Bitte melde dich an, bevor du eine Erinnerung hinterlässt.", "error");
        return;
      }
      ...
      form.reset();
      setStatus(status, "Deine Erinnerung wurde privat zur Prüfung eingereicht.", "success");
    };
  }
  ...
  async function loadPendingMemorials() {
    ...
    target.innerHTML = data.map(item => `
      <div class="review-item">
        ...
        <p>${item.public_requested ? "Öffentliche Ein Himmel voller Sterne beantragt" : "Private Erinnerung"}</p>
        <div class="review-actions">
