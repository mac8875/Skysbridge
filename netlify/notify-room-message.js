...
exports.handler = async (event) => {
  ...
  try {
    ...
    const destination = process.env.ROOM_MESSAGE_FORWARD_TO || process.env.ADMIN_EMAIL || 'together@skysbridge.org';
    ...
    const emailSent = await sendMail({ to: destination, subject, html });
    if (emailSent) {
